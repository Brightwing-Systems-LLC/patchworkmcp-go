...
//   - github.com/mark3labs/mcp-go  → RegisterFeedbackTool(server, opts)
...
var (
	patchworkURL = getEnv("PATCHWORKMCP_URL", "https://patchworkmcp.com")
	apiKey       = os.Getenv("PATCHWORKMCP_API_KEY")
	serverSlug   = os.Getenv("PATCHWORKMCP_SERVER_SLUG")
)
...
func NewFeedbackTool() mcp.Tool {
	return mcp.NewTool(ToolName,
		...
		// drop-in since mcp-go doesn't have WithArray. The API accepts both
...
// Options configures the PatchworkMCP connection.
...
// SendFeedback posts feedback to PatchworkMCP with retry logic.
//
...
func SendFeedback(ctx context.Context, args map[string]any, opts *Options) string {
	...
	endpoint := opts.url() + "/api/v1/feedback/"
	authKey := opts.key()
	var lastErr error
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			return "Feedback noted (PatchworkMCP unavailable, but your input is appreciated)."
		}
...
// NewFeedbackHandler returns a tool handler function.
// Pass nil for opts to use environment variable defaults.
func NewFeedbackHandler(opts *Options) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args := req.GetArguments()
		msg := SendFeedback(ctx, args, opts)
...
//	})
func RegisterFeedbackTool(s *server.MCPServer, opts *Options) {
	s.AddTool(NewFeedbackTool(), NewFeedbackHandler(opts))
}
